...
public :
    ThreadData(const char * command, const char * type) : 
        _command(command), _type(type), _id(0)
    {}
    
    ThreadData(const char * command, int id) : 
...
static void * command_thread(void * param)
{
    ...
    char url[1024];
    snprintf(url, sizeof(url), "%s?ip=%s&task=%d", g_notify, g_ip, data->_id);
        
    fprintf(stderr, "Info: async %s\n", data->_command.c_str());
    ...
    {
        strncpy(buf, "Error: failed to run", size);
    }
...
class SSANAgentHandler : virtual public SSANAgentIf {
 public:
  ...
  void runAsync(std::string& _return, const std::string& cmd) 
  {
    ...
    int res = pthread_create(& thread, NULL, command_thread, (void *)new ThreadData(cmd.c_str(), taskId));
    if (res)
    {
        fprintf(stderr, "Error: pthread_create %d\n", res);
        taskId = -1;
...
int main(int argc, char **argv) 
{
    ...
    if (conf)
    {
        ...
        conf_file_get(conf, "notify", g_notify, sizeof(g_notify));
        conf_file_close(conf);

        fprintf(stderr, "Info: ip=%s\n", g_ip);
        fprintf(stderr, "Info: interface=%s\n", g_web);
        fprintf(stderr, "Info: notify=%s\n", g_notify);
    }
