...
{ 
	...
	if(p!=NULL)//找到结点
	{
		...
		case 1:cout<<"请输入新编号：";cin>>number; 
			strcpy(name,p1->name);incount=p1->incount;outcount=p1->outcount;recount=p1->recount;
		...
		case 5:cout<<"请输入剩余库存数目：";cin>>recount;
			number=p1->number;strcpy(name,p1->name);incount=p1->incount;outcount=p1->outcount;
		case 6:cout<<"请输入新信息 ：\n";
			...
			//创建新货物结点
			Goods *p3;
			p3=new Goods(number,name,incount,outcount,recount); 
			p3->next=p1->next;
